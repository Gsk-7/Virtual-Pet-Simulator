document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleActionClick(e) {
        ...
        switch(action) {
            case 'start':
                if (bgMusic) {
                    bgMusic.play().catch(e => console.warn("Audio autoplay was prevented. User interaction is needed.", e));
                }
    ...
    function handleCreatePet() {
        ...
        if (name) {
            setState({
                ...
                isMuted: bgMusic ? bgMusic.muted : false,
            });
            startGameLoops();
        }
    }
    ...
    function toggleMute() {
        ...
        setState({ isMuted: bgMusic.muted });
    }
    ...
    function getEnvironmentHTML(dayNight) {
        const stars = dayNight === 'night' ? `<div class="w-4 h-4 absolute top-1/4 left-1/4 animate-fade-in-out bg-white rounded-full"></div><div class="w-3 h-3 absolute top-1/3 right-1/4 animate-fade-in-out bg-white rounded-full [animation-delay:1s]"></div>` : '';
        const sun = dayNight === 'day' ? `<div class="absolute top-10 left-10 w-16 h-16 bg-yellow-300 rounded-full animate-pulse"></div>` : '';
        const moon = dayNight === 'night' ? `<div class="absolute top-10 right-10 bg-slate-300 rounded-full" style="width: 6rem; height: 6rem;"></div>` : '';
        return stars + sun + moon + `<div class="absolute bottom-0 w-full h-1/3 bg-gradient-to-t ${dayNight === 'day' ? 'from-green-500 to-green-400' : 'from-green-800 to-green-700'}"></div>`;
    }
